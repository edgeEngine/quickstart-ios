...
extension MMKLibraryAdapter {
    ...
    class func platformAuthorizationConfig() -> MIMIKAuthConfig {
        return MIMIKAuthConfig.init(clientId: self.clientId(), redirectUrl: self.redirectUrl(), additionalScopes: ["openid", "read:me", "edge:mcm", "edge:clusters", "edge:account:associate"], authorizationRootUrl: self.authorizationRoot())
    }
    
    class func platformUnauthorizationConfig() -> MIMIKAuthConfig {
        return MIMIKAuthConfig.init(clientId: self.clientId(), redirectUrl: self.redirectUrl(), additionalScopes: nil, authorizationRootUrl: self.authorizationRoot())
    }
}
